<template>
	<header>
		<v-container>
			<div class="header__wrap pt-4 pt-md-7 d-flex align-md-center">
				<!-- <button class="header__burger mr-5 d-md-none" @click="toggleBurgerMenu">
				...
				<div class="header__burger-wrap d-md-none" @click="toggleBurgerMenu">
					<div class="header__burger-icon"
						 :class="{'header__burger-icon--opened':isBurgerMenuShown}">
						<span></span>
						<span></span>
						<span></span>
					</div>
				...
				<router-link to="/" class="logo" @click="toggleMenuLogo">
				...
				<menu-items @select="onMenuItemSelected" :isHeader="true"
							class="d-none d-md-block pt-2"/>
			</div>
		</v-container>
		<burger-menu  v-if="isBurgerMenuShown" @select="onMenuItemSelected"
					 class="d-md-none"/>
	</header>
</template>
...
<style lang="scss">
	...
	header {
		...
		left: 0;
		width: 100%;
		//background-image: url("~@/assets/header-bg-xs.png");
		height: 56px;
    ...
    @-moz-document url-prefix() {
      &:before {
        background-color: $header-bg-moz !important;
      }
    }
...
<script>
    ...
    export default {
        components: {MenuItems, BurgerMenu},
        ...
        computed: {
            isBurgerMenuShown: {
                ...
                set(v) {
                    this.$store.state.isBurgerMenuShown = v;
                }
            }
        },
        methods: {
            ...
            toggleMenuLogo() {
                // todo wtf? is it always false? the name means TOGGLE
                this.isBurgerMenuShown = true;
                this.isBurgerMenuShown = !this.isBurgerMenuShown;
            },
            onSelectLang(item) {
                this.$store.state.currentLanguageCode = item.code;
            }
        },
    }
</script>
