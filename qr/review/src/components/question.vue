<template>
  <div class="question">
    ...
    <transition name="route" mode="out-in">
      <component :key="'q' + question.ID" class="question__component" :question="question" :is="component" @click="onAnswer"/>
    </transition>
...
export default {
  name: "question",
  components: {QuestionStars, QuestionSmile, QuestionSelect, QuestionLike},
  data: () => ({
  ...
  created() {
    ...
    if (!!this.page.poll.DATE_START) {
      this.page.poll.DATE_START = this.page.poll.DATE_START.split(".").reverse().join("/")
      let dNow = new Date();
      let  dStart = new Date (new Date(this.page.poll.DATE_START).getTime() );
      if (dNow < dStart)  {
        this.$router.replace({name: 'suspended'});
        return;
      }
    ...
    if (!!this.page.poll.DATE_STOP) {
      this.page.poll.DATE_STOP = this.page.poll.DATE_STOP.split(".").reverse().join("/")
      let dNow = new Date();
      let  dStop = new Date (new Date(this.page.poll.DATE_STOP).getTime() +  (24 * 60 * 60 * 1000));
      if (dNow > dStop)  {
        this.$router.replace({name: 'suspended'});
        return;
      }
    ...
    //Проверяем наличие вопроса с таким ID
