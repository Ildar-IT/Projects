<template>
	<div class="tariff-cards anti-margin-row">
		<v-row class="ma-0 justify-center align-stretch">
			<v-col v-for="(card, idx) in pageTariffs" :key="idx" cols="12" v-if="!card.IS_DEFAULT" sm="6" md="5"
				   class="ma-0 pa-0">
				<div class="margin-left-small margin-right-small margin-bottom-main">
					<card class="d-flex flex-column justify-space-between">
						...
						<div class="flex-center" v-if="isAuthorized">
							<v-btn v-if="card.PRICE" block
								   @click="$emit('select', {tariff: selectedCard(card), period: ( period[idx]) })"
								   :disabled="currentTariff && currentTariff.ID === selectedCardID(card, idx) && currentPeriod === period[idx]"
								   class="primary">
								<pulse-loader v-if="card.loading" :loading="true" color="#fff" size="5px"/>
								<span v-else-if="currentTariff && currentTariff.ID === selectedCardID(card, idx) && currentPeriod === period[idx]">Текущий тариф</span>
								<span v-else>{{ card.BTN_LABEL }}</span>
							</v-btn>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            /**
             * Возвращает массив тарифов, которые являются "комплексными".
             * Это тарифы с одинаковым NAME, которых 2 или более.
             * @returns {*}
             */
            complexTariffs() {
            ...
            periods() {  // todo перенести в геттер
                let periodsTemp;
        ...
        methods: {
            ...
            /**
             * Возвращает CSS-класс для вывода тарифной кнопки "Подключить".
             * @param tariffID
             * @param periodIndex
             * @returns {string}
             */
            cardClass(tariffID, periodIndex) {
                // Если премиум, то смотрим также период оплаты для вывода корректного класса кнопки
                /*if (this.isPremium) {
                    if (this.currentTariff?.ID !== tariffID || this.currentPeriod !== this.period[periodIndex]) return 'primary';
                    return 'primary_disabled v-btn--disabled';
                } else {
        ...
        created() {
            // если у пользователя нет оплаченного тарифа, то показываем 1 мес и 1 опрос в селекте
            ...
            if (this.isPremium && this.isAuthorized) {
                ...
                this.currentTariff.MAX_POLLS ? this.numberOfPolls = this.currentTariff.MAX_POLLS : this.numberOfPolls = {
                    name: "1 опрос",
                    value: 1
                };
