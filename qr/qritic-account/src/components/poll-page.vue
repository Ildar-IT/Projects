	...
	<div class="poll-page layout-top">
		...
		<template v-else>
			<card-warning v-if="isPollLimited"
						  ...
						  :btnText="premiumMessage.btnText"/>
		...
		<!--                <question-card :question="question" :index="index"/>-->
		<!--            </div>-->
		<!--        </template>-->
		...
		<div v-if="!isQlean" class="flex-center margin-bottom-main">
			<v-btn large class="primary" @click="goEdit">
		...
		<h1 class="mt-16 margin-bottom-small text-center primary-text">
			Готовые шаблоны с QR-кодом
		</h1>
		<h6 class="margin-bottom-main text-center ">
			Выберите формат и напечатайте самостоятельно
		</h6>

		<templates-cards :cards="cards" :poll="this.page.item" :styles="page.styles"/>
		<template-banner/>
		<v-btn v-if="isWindowLG && !isQlean" class="primary btn-fixed-big" @click="goEdit">
			<svgicon name="pen" color="white" width="24" height="24" class="mr-2"/>
			<span>Редактировать</span>
		</v-btn>
		<v-btn v-else-if="!isQlean" @click="goEdit" fab class="primary btn-fixed-small">
...
<script>
    ...
    import TemplateBanner from "./elements/template-banner";
    ...
    export default {
        ...
        computed: {
            ...
            isQlean() {
              return this.page.item.TYPE === 'Qlean';
            },
            poll() {
