<template>
	<div class="layout-top journal margin-bottom-main">
		...
		<suspended-message v-if="isSuspendedPoll" class="margin-top-main"/>

		<template v-else-if="reviews && reviews.length">

			<v-row justify="center" class="margin-bottom-small">
				<v-col cols="4">
					<v-btn block small text class="secondary"
						   @click="downloadXLS">
			...
			<div v-if="isWindowLG">
				<!--table header-->
				<div class="flex align-center justify-space-between h7 pa-4 ml-12">
							...
							<div class="flex-space-between pt-3">
								<div class="w-865">
									...
									<div v-if="review.COMMENTS"
										 v-html="review.COMMENTS.TEXT" class=" line-break body-1 mr-4 mb-2 line-break">
									</div>
									<div v-if="review.FILE" class="body-2">
										<v-icon class="mb-1"> mdi-paperclip</v-icon>
										<a download :href="review.FILE"> {{ review.DETAIL_TEXT }} </a>
									</div>
								</div>
								<div>
			...
			<div v-else>
				<card v-for="(review, idx) in reviews" :key="idx"
					...
					<div v-if="review.COMMENTS">
						...
						<div v-html="review.COMMENTS.TEXT" class="line-break body-1 mb-2"></div>
					</div>
...
<script>
    ...
    export default {
        name: "journal",
        components: {AnswerLike, AnswerStars, AnswerSelect, AnswerSmile, PulseLoader, SuspendedMessage},
        data: () => ({
            pollId: null,
            openedPanel: -1,
            periodNumber: null,
        ...
        computed: {
            ...
            poll() {
                return this.polls?.find(el => el.ID === this.pollId);
            },
            page() {
        ...
        methods: {
            ...
            fetchJournal() {
                let params = {};
                if (this.pollId) params.hash = this.polls?.find(el => el.ID === this.pollId).QRHASH;
                if (this.periodNumber) params.period = this.periodNumber;
            ...
            downloadXLS() {
                if (this.isPremium) {
                    let params = '?period=' + this.periodNumber;
                    if (this.pollId)
        ...
        mounted() {
            this.periodNumber = Config.DEFAULT_PERIOD;
            // todo think if settings saving is needed here
            ...
            if (this.user['UF_SETTINGS'] && this.user['UF_SETTINGS']['statsPollId']) {
                if (this.polls.some(el => el.ID === this.user['UF_SETTINGS']['statsPollId'])) this.pollId = this.user['UF_SETTINGS']['statsPollId'];
                else this.pollId = null;
            }
            if (!this.pollId && this.polls.length) this.pollId = this.polls[0].ID;
        }
    }
</script>
