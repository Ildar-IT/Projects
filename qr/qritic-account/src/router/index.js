...
router.beforeEach((to, from, next) => {
    ...
    // если тариф уже не позволяет работать?
    const isSuspended = store.getters.isSuspended;
    //const allowedRoutes = ['suspended', 'tariffs', 'payment-success', 'payment-fail', 'settings', 'support', 'logout', 'login', 'subscription', 'privacy', 'subscription-rules', 'error404', 'error500'];
    if (isSuspended && to.meta?.isSuspendable) {
        // нельзя открыть этот роут пока нет оплаченного тарифа
