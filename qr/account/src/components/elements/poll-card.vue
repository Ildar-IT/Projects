<template>
    <div v-if="poll" class="poll-card margin-bottom-main"
        ...
        <card>
            <div v-if="isPollLimited || poll.STATUS == 'Y' " class="d-flex align-center justify-center mb-8 red-text">
            ...
            <div v-if="!!poll.DATE_START && !!poll.DATE_START && poll.STATUS !== 'Y' " class="d-flex align-center justify-center mb-8 gray-text">
                <svgicon name="hourglass" :fill="false" color="#B3B8CB" width="24" height="24" class="mr-4"/>
                <span class="body-2"> {{ pollLifespan }}</span>
            </div>
            <div v-if="poll.QRHASH_IS_PUBLISHED == 'Y' " class="d-flex align-center justify-center mb-8 gray-text">
                <svgicon name="qrcode" :fill="false" color="#B3B8CB" width="24" height="24" class="mr-4"/>
                <span class="body-2">Приобретён QR-код</span>
            </div>
            <div v-if="popular && poll.countReviews" class="h7 flex-center gray-text mb-4">
                Популярный опрос
            </div>
          <v-row>
    ...
    export default {
        ...
        computed: {
         ...
         /*       console.log("dateStop", dateStop);
                ...
                if (dateStop.getTime() <= new Date(now.getTime() + 60 * 60 * 24 * 1000).getTime()) { // Если до завершения опроса осталось менее 24 ч
                    ...
                    // return hh + ":" + min;
                    return "Отключение через " + new Date(new Date(diff).getTime() - 1000*60*60*2).toISOString().slice(11,16)
                }
        ...
        created() {
            ...
            if (!!this.poll.DATE_STOP) {
                this.poll.DATE_STOP = this.poll.DATE_STOP.split(".").reverse().join("/")
                let dNow = new Date();
                let  dStop = new Date (new Date(this.poll.DATE_STOP).getTime() +  (24 * 60 * 60 * 1000));
                if (dNow > dStop)  {
                    this.poll.STATUS = "Y"
                }
            }
