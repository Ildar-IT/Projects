	...
	setPage: (state, payload) => {
		if (payload.page) {
			for (const actionName in payload.page) {
				...
				Vue.set(state.session, actionName, payload.session);
		...
		if (payload.session) {
			...
			if (payload.session.auth) {
				...
				window.addEventListener('ns-brige-ready', (event) => {
					window.nsWebViewBridge.emit("gotMessage", state.session.auth);
					console.log("emit");
				});
			} else {
				localStorage.removeItem(config.STORAGE_AUTH_TOKEN);
				state.session.auth = "";
				console.log("Auth unset", localStorage);
				state.session = {}
				console.log("Auth unset actually", state.session);
				// TODO: решить, нужен ли здесьредирект на auth
			}
			if (payload.session.validationCode) {
				state.session.validationCode = payload.session.validationCode;
				localStorage.setItem('validationCode', state.session.validationCode);
				console.log("validationCode set: " + state.session.validationCode);
			}
		} else {
			// localStorage.removeItem(config.STORAGE_AUTH_TOKEN);
